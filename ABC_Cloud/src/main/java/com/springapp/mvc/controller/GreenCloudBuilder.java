...
public class GreenCloudBuilder extends CloudBuilder {
    ...
    public GreenDataCenter createDatacenter(int id, int server_nr) {
        ...
        for (int i = 1; i <= server_nr; i++) {
            hostList.add(
                    new GreenHost(i, new RamProvisionerSimple(ram),new BwProvisionerSimple(bw),
                         storage, peList,new VmSchedulerTimeShared(peList), null)
            ); // This is our machine
        }
        ...
        try {
            datacenter = new GreenDataCenter("Datacenter_" + ++id, characteristics, new VmAllocationPolicySimple(hostList), storageList, 0);
        } catch (Exception e) {
            e.printStackTrace();
        }
